...
class ResponseObject:
    ...
    def __init__(self, http_status_code: str, response: str, pandora_transaction_code: str = '', error: bool = False, pandora_error_code: int = 0, pandora_error_message: str = '') -> None:
...
class PandoraClient:
    ...
    def create_element_container(self, element_container_name: str) -> ResponseObject:
        ...
        if(request_response.status_code == self.INTERNAL_ERROR_CODE): 
            ...
            return ResponseObject(request_response.status_code, request_response.text, request_response.text[self.TRANSACTION_CODE_START_INDEX:self.TRANSACTION_CODE_END_INDEX], error=True, pandora_error_code=int(request_response.text[0:self.ERROR_CODE_LENGTH]), pandora_error_message=request_response.text[self.ERROR_MESSAGE_START_INDEX:])

        return ResponseObject(request_response.status_code, request_response.text, request_response.text[self.TRANSACTION_CODE_START_INDEX:self.TRANSACTION_CODE_END_INDEX])
